...
- (void)toggleNav:(BOOL)showNav
{
    if(showNav)
    {
        if(self.navBarView.top < 0)
        {
            ...
            [self.containerView setFrame:CGRectMake(self.containerView.left, self.navBarView.bottom, self.containerView.width, self.view.height - self.navBarView.height)];
        }
    }
    else
    {
        if(self.navBarView.top >= 0)
        {
            [self.navBarView setTransform:CGAffineTransformTranslate(self.navBarView.transform, 0, -self.navBarView.height)];
             ...
             [self.containerView setFrame:CGRectMake(self.containerView.left, 0, self.containerView.width, self.view.height)];
        }
    }
}
...
- (void)showGame
{
    ...
    [self.containerView addSubview:gameController.view];
    
    gameController.view.size = self.containerView.size;
    
    [gameController didMoveToParentViewController:self];
    
    // newController.view.left = self.containerView.width;
    gameController.view.bottom = 0;
    
    [self.scrollingBackgroundView fadeToColor:[UIColor darkGrayColor]];
    
    [UIView animateWithDuration:ANIMATION_DURATION animations:^
    {
        [currentController.view setAlpha:0.0];
    }
    completion:^(BOOL finished)
    {
        [self toggleNav:gameController.showNav];
        [self toggleProfileBar:gameController.showProfileBar];
        
         [UIView animateWithDuration:ANIMATION_DURATION animations:^
         {
             gameController.view.top = 0;
         }];
...
- (void)hideGame
{
  /*  [UIView animateWithDuration:ANIMATION_DURATION animations:^
    {
        [self.topViewController.view setBottom:0];
    }
    completion:^(BOOL finished)
    {
        [self.scrollingBackgroundView fadeToScroll];
        [self popViewController];
    ...
    completion:^(BOOL finished)
    {
         ...
         completion:^(BOOL finished)
         {
              ...
              if(finished)
              {
                  ...
                  [self toggleNav:self.topViewController.showNav];
                  [self toggleProfileBar:self.topViewController.showProfileBar];
              }
          }];
     }];
